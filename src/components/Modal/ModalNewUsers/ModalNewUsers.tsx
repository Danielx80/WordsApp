...
const ModalNewUser = ({ size, textHeader }: ModalNewProps) => {
  ...
  const validationSchema = object({
    ...
    first_name: string().required('El nombre es requerido').min(1, 'El nombre tiene que tener almenos un caracter').max(10, 'El nombre no pued esuperar los 10 caracteres'),
    language: string().required('Debes seleccionar un lenguaje'),
    last_name: string().required('El apellido es requerido').min(1, 'El apellido tiene que tener almenos un carácter').max(20, ' El apellido no pued esuperar los 20 caracteres'),
    telephone: string().min(10, 'Minimo 10 caracteres').max(10, 'Maximo 10 caracteres').required('El telefono es requerido'),
  ...
  const formik = useFormik<IUser>({
    ...
    onSubmit: userFormik => {
      mutate({ ...userFormik, /*isAdmin* : user.isAdmin*/ })
